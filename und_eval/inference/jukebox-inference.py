...
input_dir = cfg.model.jukebox.input_dir
output_dir = cfg.model.jukebox.output_dir
state_dir = cfg.model.jukebox.state_dir
...
def processFile(enc, batch):
    ...
    target_samples = int(min(total_duration, maxDur) * sample_rate)
    trimmed_waveform = waveform[:, :target_samples]
    
    res = enc(trimmed_waveform.numpy(), sample_rate, batch["audio_length"])
    # print(res[1].shape)
...
def process(rank:int, world_size:int, host:dict, data_loader_num_workers:int=2):
    ...
    enc = jukebox.encoder.Jukebox(device=device)
