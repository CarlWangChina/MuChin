...
from dataset import VecDatasetJukeBox
from torch.nn import BCEWithLogitsLoss  
from tqdm import tqdm  
from omegaconf import OmegaConf
cfg = OmegaConf.load('configs/train.yaml')

num_epochs = cfg.model.jukebox_s1.num_epochs  # 设置训练轮数  
save_path = cfg.model.jukebox_s1.save_path  # 指定模型保存路径  
device = cfg.model.jukebox_s1.device
dataset_index = cfg.model.jukebox_s1.dataset_index
loss_path = cfg.model.jukebox_s1.loss_path
output_len = cfg.model.jukebox_s1.output_len

ds = VecDatasetJukeBox(path=dataset_index, output_len=output_len,replace=cfg.model.jukebox_s1.dataset)  
data_loader = torch.utils.data.DataLoader(ds, batch_size=cfg.model.jukebox_s1.batch_size, shuffle=True, num_workers=36)
  
model = MLP(4800, 8192, output_len).to(device)
