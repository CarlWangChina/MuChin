...
num_epochs = cfg.model.mert330_s1.num_epochs  # 设置训练轮数  
save_path = cfg.model.mert330_s1.save_path  # 指定模型保存路径  
device = cfg.model.mert330_s1.device
dataset_index = cfg.model.mert330_s1.dataset_index
loss_path = cfg.model.mert330_s1.loss_path
output_len = cfg.model.mert330_s1.output_len
dataset_pt = cfg.model.mert330_s1.dataset_pt

ds = VecDatasetSearch(path=dataset_index, output_len=output_len, dir_path=dataset_pt,replace=cfg.model.mert330_s1.dataset)  
data_loader = torch.utils.data.DataLoader(ds, batch_size=cfg.model.mert330_s1.batch_size, shuffle=True)
  
model = MLP(1024, 8192, output_len).to(device)
