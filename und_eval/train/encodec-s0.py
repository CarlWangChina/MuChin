...
from dataset import VecDatasetEncodec
from torch.nn import BCEWithLogitsLoss  
from tqdm import tqdm  
from omegaconf import OmegaConf
cfg = OmegaConf.load('configs/train.yaml')

num_epochs = cfg.model.encodec_s0.num_epochs  # 设置训练轮数  
save_path = cfg.model.encodec_s0.save_path  # 指定模型保存路径  
device = cfg.model.encodec_s0.device
dataset_index = cfg.model.encodec_s0.dataset_index
loss_path = cfg.model.encodec_s0.loss_path
output_len = cfg.model.encodec_s0.output_len

ds = VecDatasetEncodec(path=dataset_index, output_len=output_len,replace=cfg.model.encodec_s0.dataset)  
data_loader = torch.utils.data.DataLoader(ds, batch_size=cfg.model.encodec_s0.batch_size, shuffle=True)
  
model = MLP(19200, 8192, output_len).to(device)
